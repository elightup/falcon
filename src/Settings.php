...
class Settings {
			...
			<div class="e-body wrap">
				...
				<div class="e-wrapper">
					<div class="e-content e-box">
						<nav class="e-tabs" aria-label="<?php esc_attr_e( 'Settings sections', 'falcon' ); ?>">
							<?php foreach ( $groups as $group_key => $group ) : ?>
								<a href="#<?php echo esc_attr( $group_key ); ?>" data-group="<?php echo esc_attr( $group_key ); ?>" class="e-tab"><?php echo esc_html( $group['label'] ); ?></a>
							<?php endforeach; ?>
							...
							<div class="e-subTabsBar">
								<?php foreach ( $groups as $group_key => $group ) : ?>
									<?php if ( empty( $group['tabs'] ) ) : ?>
										<?php continue; ?>
									<?php endif; ?>
									<nav class="e-subTabs" data-group="<?php echo esc_attr( $group_key ); ?>" aria-label="<?php echo esc_attr( sprintf( /* translators: %s: settings section name */ __( '%s sub-sections', 'falcon' ), $group['label'] ) ); ?>">
										<?php foreach ( $group['tabs'] as $tab_key => $tab_label ) : ?>
											<a href="#<?php echo esc_attr( "$group_key/$tab_key" ); ?>" data-group="<?php echo esc_attr( $group_key ); ?>" data-tab="<?php echo esc_attr( $tab_key ); ?>" class="e-subTab"><?php echo esc_html( $tab_label ); ?></a>
										<?php endforeach; ?>
									</nav>
								<?php endforeach; ?>
							</div>
					...
					<div class="e-sidebar">
						<div class="e-widget e-box">
							...
							<div class="e-widget_actions">
								...
								<a href="<?php echo esc_url( $url ) ?>" class="button"><?php esc_html_e( 'Export', 'falcon' ) ?></a>
						...
						<div class="e-widget e-box">
							...
							<p><?php echo wp_kses_post( sprintf( __( 'If you like Falcon, please share it with your friends or <a href="%1$s" target="_blank">write a review</a> to help us spread the word. We also want to hear <a href="%2$s" target="_blank">your feedback</a> to improve the plugin.', 'falcon' ), 'https://wordpress.org/support/plugin/falcon/reviews/', 'https://elu.to/fse' ) ); ?></p>
						</div>
						<div class="e-widget e-box">
	...
	private function get_tab_groups(): array {
		return apply_filters( 'falcon_settings_tab_groups', [
			'performance' => [
				'label' => __( 'Performance', 'falcon' ),
				'tabs'  => null,
			],
			'security'    => [
				'label' => __( 'Security', 'falcon' ),
				'tabs'  => null,
			],
			'cleanup'     => [
				'label' => __( 'Cleanup', 'falcon' ),
				'tabs'  => [
					'header'    => __( 'Header', 'falcon' ),
					'admin'     => __( 'Admin', 'falcon' ),
					'frontend'  => __( 'Frontend', 'falcon' ),
					'database'  => __( 'Database', 'falcon' ),
				],
			],
			'content'     => [
				'label' => __( 'Content', 'falcon' ),
				'tabs'  => [
					'editor'     => __( 'Editor', 'falcon' ),
					'comments'   => __( 'Comments', 'falcon' ),
					'media'      => __( 'Media', 'falcon' ),
					'publishing' => __( 'Publishing', 'falcon' ),
				],
			],
			'system'      => [
				'label' => __( 'System', 'falcon' ),
				'tabs'  => null,
			],
			'email'       => [
				'label' => __( 'Email', 'falcon' ),
				'tabs'  => [
					'notifications' => __( 'Notifications', 'falcon' ),
					'delivery'      => __( 'Delivery', 'falcon' ),
				],
			],
		] );
	}
	...
	private function get_tab_panes(): array {
		$tab_panes = [];
		foreach ( $this->get_tab_groups() as $group_key => $group ) {
			if ( empty( $group['tabs'] ) ) {
				$tab_panes[ $group_key ] = $this->get_tab_pane( $group_key );
				continue;
			}
			foreach ( array_keys( $group['tabs'] ) as $tab_key ) {
				$id = "$group_key/$tab_key";
				$tab_panes[ $id ] = $this->get_tab_pane( $group_key, $tab_key );
			}
		}
		return $tab_panes;
	}

	public function get_tab_pane( string $group, ?string $tab = null ): string {
		$path = null === $tab
			? FALCON_DIR . "/views/settings/groups/$group.php"
			: FALCON_DIR . "/views/settings/groups/$group/$tab.php";

		ob_start();
		printf(
			'<div class="e-tabPane" data-group="%1$s"%2$s>',
			esc_attr( $group ),
			null === $tab ? '' : sprintf( ' data-tab="%s"', esc_attr( $tab ) )
		);
		if ( file_exists( $path ) ) {
			include $path;
		}
		$hook = null === $tab ? "falcon_settings_tab_$group" : "falcon_settings_tab_{$group}_$tab";
		do_action( $hook, $this );
		echo '</div>';
		return ob_get_clean();
	}
	...
	public function checkbox( string $name, string $label, string $description = '' ): void {
		...
		<div class="featureBox">
			<label class="featureBox_switch">
				<input class="featureBox_input" type="checkbox" name="falcon[features][]" value="<?php echo esc_attr( $name ); ?>"<?php checked( self::is_feature_active( $name ) ) ?>>
				<span class="featureBox_icon"></span>
			</label>
			<div class="featureBox_body">
				<div class="featureBox_title"><?php echo wp_kses_post( $label ); ?></div>
